  ...
  const handleContinue = async () => {
    ...
    try {
      if (paymentMethod === "card") {
        ...
        sessionStorage.setItem("walletRechargeResponse", JSON.stringify(response.payment))
        return
        ...
        setShowSuccessModal(true)
        sessionStorage.setItem("walletRechargeResponse", JSON.stringify(response))
        return
      }
    } catch (error: any) {
    ...
    <V7Layout>
      ...
      <AlertDialog open={showSuccessModal}>
        <AlertDialogContent className="text-center max-w-lg w-full p-10 rounded-2xl shadow-2xl bg-gray-50 border border-gray-200">
          <h2 className="text-2xl font-bold mb-6 text-green-700">تمت العملية بنجاح</h2>
          {modalData && (
            <div className="space-y-3 text-right mx-auto max-w-md">
              {modalData.amount_format && (
                <div><span className="font-semibold">المبلغ:</span> <span className="font-normal">{modalData.amount_format}</span></div>
              )}
              {modalData.currency && (
                <div><span className="font-semibold">العملة:</span> <span className="font-normal">{modalData.currency}</span></div>
              )}
              {modalData.status && (
                <div><span className="font-semibold">الحالة:</span> <span className="font-normal">{modalData.status}</span></div>
              )}
              {modalData.transaction_url && (
                <div><span className="font-semibold">رابط العملية:</span> <a href={modalData.transaction_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline break-all font-normal">{modalData.transaction_url}</a></div>
              )}
              {modalData.company && (
                <div><span className="font-semibold">الشركة:</span> <span className="font-normal">{modalData.company}</span></div>
              )}
              {modalData.name && (
                <div><span className="font-semibold">الاسم:</span> <span className="font-normal">{modalData.name}</span></div>
              )}
              {/* Source details */}
              {modalData.source && (
                <div className="mt-6 p-4 rounded-lg bg-gray-100 border border-gray-200">
                  <div className="font-bold mb-2 text-base text-gray-700">تفاصيل البطاقة</div>
                  {modalData.source.type && (
                    <div><span className="font-semibold">نوع البطاقة:</span> <span className="font-normal">{modalData.source.type}</span></div>
                  )}
                  {modalData.source.company && (
                    <div><span className="font-semibold">الشركة:</span> <span className="font-normal">{modalData.source.company}</span></div>
                  )}
                  {modalData.source.name && (
                    <div><span className="font-semibold">الاسم:</span> <span className="font-normal">{modalData.source.name}</span></div>
                  )}
                  {modalData.source.number && (
                    <div><span className="font-semibold">رقم البطاقة:</span> <span className="font-normal">{modalData.source.number}</span></div>
                  )}
                 
                  {modalData.source.issuer_name && (
                    <div><span className="font-semibold">اسم البنك المصدر:</span> <span className="font-normal">{modalData.source.issuer_name}</span></div>
                  )}
                  {modalData.source.issuer_country && (
                    <div><span className="font-semibold">بلد البنك المصدر:</span> <span className="font-normal">{modalData.source.issuer_country}</span></div>
                  )}
                  {modalData.source.issuer_card_type && (
                    <div><span className="font-semibold">نوع البطاقة المصدرة:</span> <span className="font-normal">{modalData.source.issuer_card_type}</span></div>
                  )}
                  {modalData.source.issuer_card_category && (
                    <div><span className="font-semibold">فئة البطاقة المصدرة:</span> <span className="font-normal">{modalData.source.issuer_card_category}</span></div>
                  )}
                  {modalData.source.transaction_url && (
                    <div><span className="font-semibold">رابط العملية:</span> <a href={modalData.source.transaction_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline break-all font-normal">{modalData.source.transaction_url}</a></div>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="flex justify-center mt-8">
            <AlertDialogAction onClick={handleModalOk} className="px-10 py-3 text-lg bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow">موافق</AlertDialogAction>
          </div>
        </AlertDialogContent>
      </AlertDialog>
      <div className="rounded-xl shadow-md v7-neu-card p-6">
        ...
        <div className="space-y-8">
          ...
          {paymentMethod === "bank" && (
            <div className="space-y-4">
              <h2 className="text-xl font-bold text-right">إيصال التحويل البنكي</h2>
              <p className="text-right text-sm text-gray-600">قم برفع صورة إيصال التحويل البنكي</p>
              
              <div className="flex justify-center">
                <label className={`cursor-pointer border-2 border-dashed rounded-lg p-8 text-center transition-all hover:border-blue-500 ${
                  isDarkMode ? "border-gray-600 bg-dark-card" : "border-gray-300 bg-gray-50"
                }`}>
                  <input
                    type="file"
                    accept="image/*"
                  ...
                  {transferImagePreview ? (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-gray-500">JPG, PNG, GIF حتى 5MB</p>
                    </div>
                  )}
                </label>
              </div>
          ...
          {paymentMethod === "card" && (
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-right text-sm font-medium mb-2">اسم حامل البطاقة</label>
                  <Input
                    value={cardName}
                ...
                <div>
                  <label className="block text-right text-sm font-medium mb-2">رقم البطاقة</label>
                  <Input
                    value={cardNumber}
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-right text-sm font-medium mb-2">CVC</label>
                    <Input
                      value={cardCvc}
                  ...
                  <div>
                    <label className="block text-right text-sm font-medium mb-2">الشهر</label>
                    <Input
                      value={cardMonth}
                  ...
                  <div>
                    <label className="block text-right text-sm font-medium mb-2">السنة</label>
                    <Input
                      value={cardYear}
          ...
          <div className="flex justify-center mt-8">
            <Button
              ...
              disabled={
                ...
                (paymentMethod === "card" && (!cardName || !cardNumber || !cardCvc || !cardMonth || !cardYear)) ||
                isLoading
              }
