  ...
  const onSubmit = async (data: any) => {
    try {
      const payload = {
        ...
        shapmentingType: data.shipmentType === "الدفع المسبق" ? "Dry" : data.shipmentType === "الدفع عند الاستلام" ? "COD" : data.shipmentType,
    ...
    <>
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="flex items-center justify-start mb-8">
          <h2 className="text-2xl font-bold text-[#1a365d] ml-3">معلومات الطلب</h2>
            ...
            <div className="v7-neu-card-inner px-6 py-6 w-full min-w-[180px] flex items-center">
              <div className="flex items-center justify-between w-full">
                <Button
                  ...
                  onClick={() => setValue("Parcels", Math.max(1, (Number(watch('Parcels')) || 1) - 1))}
                >
                  <span className="text-lg font-bold">-</span>
                </Button>
                <div className="text-2xl font-bold text-[#3498db]">{watch('Parcels') || 1}</div>
                <Button
                  type="button"
                  variant="outline"
                  size="icon"
                  className="h-8 w-8 rounded-full"
                  onClick={() => setValue("Parcels", (Number(watch('Parcels')) || 1) + 1)}
                >
                  <span className="text-lg font-bold">+</span>
                </Button>
              </div>
        ...
        <div className="w-full flex justify-center mt-8">
          <div className="w-full max-w-3xl bg-[#f6fbff] border border-[#b6d6f6] rounded-2xl px-6 py-4 flex flex-col gap-2 shadow-sm">
            <div className="flex items-center justify-between mb-1">
              <span className="font-bold text-[#1a365d] text-base text-right w-full flex items-center gap-2 justify-start">
                <span className="flex items-center justify-center w-8 h-8 rounded-full bg-white border border-[#b6d6f6]">
                  <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M12 2C7.03 2 3 6.03 3 11c0 5.25 7.05 10.74 8.13 11.54.53.39 1.21.39 1.74 0C13.95 21.74 21 16.25 21 11c0-4.97-4.03-9-9-9zm0 17.88C10.19 18.09 5 14.14 5 11c0-3.87 3.13-7 7-7s7 3.13 7 7c0 3.14-5.19 7.09-7 8.88zM11 14h2v2h-2zm0-8h2v6h-2z" fill="#3498db"/></svg>
                </span>
            ...
            <div className="bg-white/60 rounded-xl px-4 py-3 text-sm text-[#6b7a90] border border-[#e3eaf3] text-right">
              بالمتابعة، أنت توافق على <a href="#" className="text-[#3498db] font-bold underline hover:text-blue-700">شروط الخدمة</a> و <a href="#" className="text-[#3498db] font-bold underline hover:text-blue-700">سياسة الخصوصية</a> الخاصة بنا
            </div>
          </div>
        </div>
...
function InputField({ name, label, register, error, type = "text", readOnly, loading = false, placeholder, icon: Icon }: any) {
    ...
    <div className="space-y-2">
      ...
      <Input
        ...
        className={cn(
          ...
          error ? "border-red-500 focus-visible:ring-red-500" : "border-[#e3eaf3] focus:border-[#3498db]"
        )}
        placeholder={placeholder}
      />
      {loading && <div className="text-xs text-blue-500">جاري جلب بيانات الطلب...</div>}
